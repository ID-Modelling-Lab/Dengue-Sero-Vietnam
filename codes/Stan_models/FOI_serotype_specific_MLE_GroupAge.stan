...
int veca_mean[a]; // vector age mean [ group data by groups of age]
...
pneg[ar]=exp(-sum(lambda)*veca_mean[ar]);// the prob of neg at each age group and year


//Original probability calculations
pprimsero1[ar]=(exp(-(lambda[2]+lambda[3]+lambda[4])*veca_mean[ar]))*(1-exp(-lambda[1]*veca_mean[ar]));
pprimsero2[ar]=(exp(-(lambda[1]+lambda[3]+lambda[4])*veca_mean[ar]))*(1-exp(-lambda[2]*veca_mean[ar]));
pprimsero3[ar]=(exp(-(lambda[1]+lambda[2]+lambda[4])*veca_mean[ar]))*(1-exp(-lambda[3]*veca_mean[ar]));
pprimsero4[ar]=(exp(-(lambda[1]+lambda[2]+lambda[3])*veca_mean[ar]))*(1-exp(-lambda[4]*veca_mean[ar]));
