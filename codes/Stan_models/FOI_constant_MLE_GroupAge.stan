data {
  ...
  int datap[6,a]; //vector of sero status (negative, primary (DENV1-4), and secondary for all age groups), split by year
	...
	int veca_mean[a]; // vector of age mean (in case data input is grouped by groups of age)
...
for (ar in 1:a) {
pneg[ar]=exp(-lambda*veca_mean[ar]*4);// the prob of neg at each age group and year
pprimsero1[ar]=(exp(-3*lambda*veca_mean[ar])*(1-exp(-lambda*veca_mean[ar])));
